...
def insert_search_menu_action(anki_web_view, m):
    ...
    for SE in SEARCH:
        ...
        truncated = (
            selected[:40] + '..') if len(selected) > 40 else selected
        a = m.addAction('Search %s for "%s" ' % (
            SEARCH_PROVIDER, truncated))
        if len(selected) == 0:
            a.setDisabled(True)
        a.connect(
            a, SIGNAL('triggered()'),
            lambda wv=anki_web_view,
            loc=SEARCH_URL: on_search_for_selection(wv, loc))
