...
#
# The image is enlarged up to a maximum of 95% of the height or width of the card's window.
#
...
if __name__ == '__main__':
    print("This is _Zooming add-on for the Anki program and it can't be run directly.")
...
def zoom_info():
    showText('<table><tbody>\n'+\
        '<tr><td align=right><big>deck_browser_standard_zoom = </big>'+\
        '</td><td><big><b>'+str(deck_browser_current_zoom)+'</b></big></td></tr>\n'+\
...
def zoom_images(act):
    ...
    if ZOOM_IMAGES:
        showWarning('Чтобы изменения вступили в силу —<br> пожалуйста, <b>перезапустите Anki</b>' if lang=='ru' \
...
def zoom_setup_menu():
    ...
    zoom_images_action = QAction('Масштаб &картинок менять' if lang == 'ru' else _('&Zoom Images'), mw)
    ...
    mw.connect(zoom_images_action, SIGNAL('triggered()'), lambda AKT=zoom_images_action: zoom_images(AKT) )
    ...
    reset_zoom_action = QAction('Масштаб на&чальный ( ='+str(deck_browser_standard_zoom)+' ='+str(overview_standard_zoom)+' ='+str(reviewer_standard_zoom)+' )' if lang == 'ru' else _('&Reset Initial ')+'( ='+str(deck_browser_standard_zoom)+' ='+str(overview_standard_zoom)+' ='+str(reviewer_standard_zoom)+' )', mw)
...
def load_toolbarz_visible():
    global deck_browser_current_zoom, overview_current_zoom, reviewer_current_zoom
    global zoom_images_action, ZOOM_IMAGES
...
if os.path.exists(os.path.join(mw.pm.addonFolder(), 'Zoom.py')):
   showCritical( 'Найдено старое дополнение <b>Zoom</b> <br><br> Оно несовместимо с новым дополнением <i>_Zooming</i><br> &nbsp; Для нормальной работы:<br> &nbsp;  &nbsp; - удалите старое дополнение<br>  &nbsp; &nbsp; &nbsp; - перезапустите Anki.' if lang == 'ru' else 'The old version of <b>Zoom</b> add-on <br><br> is incompatible with new edition of <i>_Zooming</i>. <br> Please, remove old file and restart Anki.' )
