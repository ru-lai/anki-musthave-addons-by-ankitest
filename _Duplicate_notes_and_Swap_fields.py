...
if __name__ == '__main__':
    print("""This is _Duplicate_notes_and_Swap_fields_of_Selected_cards.py add-on \
...
DUPE_TAG = False
DUPE_TAG = datetime.datetime.now().strftime('double::dupe-%Y-%m-%d') #-%H:%M:%S')
...
def createDuplicate(self):
    ...
    if not cids:
        ...
        return
    SQL = 'select DISTINCT did from cards where id in (%s)'%(','.join(str(i) for i in cids))
    ...
    for did in dids:
      ...
      if deck['dyn']:
        ...
        # All cards are in Filtered Decks — get Name of destination deck from user.
        deckName = ''

    (deckName, retv) = getText(_('Enter the name of target deck:'), default=deckName)
...
def setupMenu(self):
    ...
    dup_action = menu.addAction('Дублировать записи и обменять поля' if lang=='ru' \
        else _('Duplicate notes and Swap fields'))
    dup_action.setShortcut(QKeySequence(HOTKEY['dupe'][0]))
    self.connect(dup_action, SIGNAL('triggered()'), lambda s=self: createDuplicate(s))
